 ...
 document.addEventListener('DOMContentLoaded', () => {
  ...
  customElements.define('x-view', class extends HTMLElement {
    ...
    constructor (opts = {}) {
      ...
      const begin = e => this.onBegin(e, this.begin)
      const during = e => this.onDuring(e, this.during)
      const end = e => this.onEnd(e, this.end)

      this.addEventListener('mousedown', begin)
      this.addEventListener('touchstart', begin)
     ...
     */
    getTransform () {
      const style = window.getComputedStyle(this).transform
      let currentX = 0
      let currentY = 0
    ...
    updatePosition (axis, pos) {
      ...
      if (this.opts.updateType === 'relative') {
        ...
        this.style.transform = `${property}(${posInViewportUnits}${unit})`
      } else if (this.opts.updateType === 'absolute') {
        this.style.transform = `${property}(${pos}px)`
      }
